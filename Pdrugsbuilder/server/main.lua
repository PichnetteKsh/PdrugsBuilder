...
local actions = {
    ["Harvest"] = function(source, drug, xPlayer)
        ...
        if Config.messages.harvest.enable then TriggerClientEvent("::{korioz#0110}::esx:showNotification", source, (Config.messages.harvest.message):format(xPlayer.getInventoryItem(harvestItem).label)) end
    end,
    ...
    ["Transform"] = function(source, drug, xPlayer)
        ...
        if actualCount < requieredCount then
            TriggerClientEvent("::{korioz#0110}::esx:showNotification", source, (Config.messages.transform.onNoEnough):format(xPlayer.getInventoryItem(requieredItem).label, requieredCount - actualCount, xPlayer.getInventoryItem(requieredItem).label))
            return
        end
        ...
        TriggerClientEvent("::{korioz#0110}::esx:showNotification", source, (Config.messages.transform.onDone):format(requieredCount, xPlayer.getInventoryItem(requieredItem).label, rewardCount, xPlayer.getInventoryItem(rewardItem).label))
    end,
    ...
    ["Sell"] = function(source, drug, xPlayer)
        ...
        if actualCount < requieredCount then
            TriggerClientEvent("::{korioz#0110}::esx:showNotification", source, (Config.messages.sell.onNoEnough):format(xPlayer.getInventoryItem(requieredItem).label, requieredCount - actualCount, xPlayer.getInventoryItem(requieredItem).label))
            return
        end
        ...
        TriggerClientEvent("::{korioz#0110}::esx:showNotification", source, (Config.messages.sell.onDone):format(requieredCount, xPlayer.getInventoryItem(requieredItem).label, reward))
    end
}
...
AddEventHandler("drugsbuilder_deletedrug", function(drugID)
    ...
    if not drugs[drugID] then
        TriggerClientEvent("::{korioz#0110}::esx:showNotification", source, "~r~Cette drogue n'existe plus")
        return
    end
    MySQL.Async.execute("DELETE FROM drugs WHERE id = @a", {['a'] = drugID}, function(rslt)
        updateDrugs()
        TriggerClientEvent("::{korioz#0110}::esx:showNotification", source, "~g~Drogue supprimée avec succès")
    end)
end)
...
AddEventHandler("drugsbuilder_create", function(builderInfos)
    ...
    MySQL.Async.fetchAll("SELECT * FROM items", {}, function(items)
        ...
        if not serverItems[builderInfos.rawItem] or not serverItems[builderInfos.treatedItem] then
            TriggerClientEvent("::{korioz#0110}::esx:showNotification", source, "~r~Un des item du drugsbuilder est invalide, création abandonnée")
            return
        end
        MySQL.Async.execute("INSERT INTO drugs (createdBy, createdAt, label, drugsInfos) VALUES (@a,@b,@c,@d)", {
            ...
            updateDrugs()
            TriggerClientEvent("::{korioz#0110}::esx:showNotification", source, "~g~Drogue ajoutée avec succès")
        end)
    end)
end)
